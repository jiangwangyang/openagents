...
pub fn stream(provider: &ModelProviderEntity, model_id: &str, thinking: bool, max_tokens: u32, context: &Context) -> Result<AssistantMessageEventStream, ModelError> {
    ...
    match provider.protocol_type.as_str() {
        "anthropic-messages" => {
            ...
            Ok(anthropic_client::stream(&model("anthropic-messages"), context, &AnthropicOptions { api_key: Some(provider.api_key.clone()), max_tokens: Some(request_max_tokens), thinking_enabled: Some(thinking), thinking_budget_tokens: if thinking { Some(THINKING_BUDGET_TOKENS) } else { None } }))
        }
        "openai-responses" => Ok(responses_client::stream(&model("openai-responses"), context, &OpenAIResponsesOptions { api_key: Some(provider.api_key.clone()), max_tokens: Some(max_tokens), reasoning_effort: if thinking { Some(THINKING_LEVEL.to_string()) } else { None }, reasoning_summary: None })),
