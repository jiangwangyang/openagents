...
pub async fn static_file(uri: Uri) -> impl IntoResponse {
    ...
    match StaticAssets::get(path) {
        Some(content) => ([(header::CONTENT_TYPE, guess_mime_type(path))], content.data).into_response(),
...
pub async fn list_directory(Query(query): Query<DirListQuery>) -> Result<impl IntoResponse, AppError> {
    let path = if query.path.is_empty() { std::path::PathBuf::from(config::home_dir()) } else { std::path::PathBuf::from(&query.path) };
    ...
    while let Some(entry) = entries.next_entry().await.map_err(|e| AppError::Internal(e.into()))? {
        ...
        if is_dir {
            ...
            directories.push(json!({"name": name, "path": resolved.to_string_lossy()}));
        }
    }
    ...
    Ok(axum::Json(json!({"current_path": current_path.to_string_lossy(), "parent_path": parent_path.map(|p| p.to_string_lossy().to_string()).unwrap_or_default(), "directories": directories})))
}
