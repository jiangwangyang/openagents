...
// 查询任务执行循环是否正在运行: entry 存在即运行中(循环退出前先从 map 移除, 与 start_task 的存在判定口径一致)
pub fn is_task_running(state: &AppState, task_id: i64) -> bool {
    state.task_loops.contains_key(&task_id)
}
...
pub async fn append_task_user_message(state: &AppState, task: &TaskEntity, message: &str, allow_create: bool) -> Result<Option<i64>, AppError> {
    let latest = conversation_repository::get_latest_task_conversation_state(&state.db, task.id).await?;
    let conversation_id = match latest {
        Some(l) if l.agent_id.is_none() => l.id,
        _ if allow_create => conversation_repository::add_conversation(&state.db, &format!("{}-User", task.title), &task.work_dir, "", Some(task.id), None, None).await?,
        _ => return Ok(None),
    };
    // content 列存整条 pi 消息 JSON
    let user_message = conversation_service::user_text_message(message);
    conversation_repository::add_conversation_messages(&state.db, conversation_id, &[serde_json::to_value(&user_message)?]).await?;
    Ok(Some(conversation_id))
}

// 后台执行循环
async fn run_task(state: AppState, task_id: i64, agent_id: i64, stop_rx: tokio::sync::watch::Receiver<bool>) {
    ...
    if let Err(e) = result {
        ...
        if let Err(err) = task_repository::update_task_status(&state.db, task_id, TASK_STATUS_FAILED).await {
            tracing::error!("Task status update failed: task_id={} error={}", task_id, err);
        }
    }
...
// 实际任务循环逻辑
async fn do_run_task(state: &AppState, task_id: i64, agent_id: i64, mut stop_rx: tokio::sync::watch::Receiver<bool>) -> anyhow::Result<()> {
    ...
    let agent = agent_repository::get_agent(&state.db, agent_id).await?.map(|d| d.agent);
    let (task, agent) = match (task, agent) {
        (Some(t), Some(a)) => (t, a),
        _ => return Ok(()),
    };
    conversation_repository::add_conversation(&state.db, &format!("{}-{}", task.title, agent.name), &task.work_dir, &agent.prompt, Some(task_id), Some(agent_id), None).await?;
    ...
    loop {
        ...
        if need_handover_reminder {
            ...
            if consecutive_reminders > MAX_HANDOVER_REMINDERS {
                tracing::warn!("Task loop ended: task_id={} conversation_id={} reason=handover_reminder_limit", task_id, latest.id);
                // 连续不交接超限: 置为运行失败
                task_repository::update_task_status(&state.db, task_id, TASK_STATUS_FAILED).await?;
                return Ok(());
            }
            tracing::info!("Task round ended without handover, reminding agent: task_id={} conversation_id={} reminder={}/{}", task_id, latest.id, consecutive_reminders, MAX_HANDOVER_REMINDERS);
        }
        ...
        // 模型配置从当前对话的 Agent 读取
        let agent = agent_repository::get_agent(&state.db, latest_agent_id).await?.ok_or_else(|| anyhow::anyhow!("agent not found"))?.agent;
        ...
        // 等待对话完成: 订阅通知并等待, 订阅后先复查通道是否已关闭避免错过完成信号; 同时监听任务停止信号
        if let Some(conv_state) = conversation_service::get_conversation_state(state, latest.id) {
            // 通道已关闭说明对话刚好完成, 无需等待
            let receiver = conv_state.read().await.notify.as_ref().map(|notify| notify.subscribe());
