...
pub async fn get_model_provider(State(state): State<AppState>, Path(provider_id): Path<i64>) -> Result<Json<ModelProviderEntity>, AppError> {
    let provider = model_provider_repository::get_model_provider(&state.db, provider_id).await?.ok_or_else(|| AppError::NotFound("Model provider not found".to_string()))?;
    Ok(Json(provider))
}
...
// 新增模型提供商, 返回自增 id
pub async fn add_model_provider(State(state): State<AppState>, Json(req): Json<ModelProviderRequest>) -> Result<Json<i64>, AppError> {
    let id = model_provider_repository::add_model_provider(&state.db, &req.name, &req.protocol_type, &req.base_url, &req.api_key).await?;
    Ok(Json(id))
}
...
pub async fn delete_model_provider(State(state): State<AppState>, Path(provider_id): Path<i64>) -> Result<(), AppError> {
    match model_provider_repository::delete_model_provider(&state.db, provider_id).await? {
        ...
        DeleteResult::Referenced => Err(AppError::Conflict("Model provider is referenced by agents".to_string())),
    }
}
...
pub async fn list_provider_models(State(state): State<AppState>, Path(provider_id): Path<i64>) -> Result<Json<Vec<String>>, AppError> {
    let provider = model_provider_repository::get_model_provider(&state.db, provider_id).await?.ok_or_else(|| AppError::NotFound("Model provider not found".to_string()))?;
    let models = ai::client::list_models(&provider).await.map_err(|e| AppError::Internal(e.into()))?;
    Ok(Json(models))
}
