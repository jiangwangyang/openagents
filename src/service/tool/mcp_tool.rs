...
pub async fn connect_mcp_server(db: &SqlitePool, server_id: i64) -> Result<RunningService<RoleClient, ClientInfo>, String> {
    let server = mcp_server_repository::get_mcp_server(db, server_id).await.map_err(|e| format!("failed to get MCP server: {}", e))?.ok_or_else(|| "MCP server not found".to_string())?;
            ...
            if let Some(Value::Object(headers_map)) = &server.headers {
                ...
                for (k, v) in headers_map {
                    if let Some(val_str) = v.as_str() {
                        if let (Ok(header_name), Ok(header_value)) = (k.parse::<http::HeaderName>(), val_str.parse::<http::HeaderValue>()) {
...
async fn connect_by_id(db: &SqlitePool, server_id: &str) -> Result<RunningService<RoleClient, ClientInfo>, ToolResult> {
    let server_id = server_id.parse::<i64>().map_err(|_| (format!("Invalid server id {}", server_id), true))?;
    connect_mcp_server(db, server_id).await.map_err(|e| (format!("Failed to connect MCP server {}: {}", server_id, e), true))
}
...
pub async fn execute(cmd_and_args: &[String], db: &SqlitePool) -> ToolResult {
    ...
    match args.as_slice() {
        ...
        ["mcp", "server", "list"] => {
            ...
            let result: Vec<serde_json::Value> = servers.iter().map(|server| serde_json::json!({"id": server.id, "name": server.name, "description": server.description})).collect();
        ...
        ["mcp", "server", server_id, "tool", "list"] => {
            ...
            let result = async {
                match service.peer().list_all_tools().await {
                    Ok(tools) => {
                        let tools: Vec<serde_json::Value> = tools.iter().map(|tool| serde_json::json!({"name": tool.name, "description": tool.description})).collect();
                        (serde_json::to_string(&tools).unwrap_or_default(), false)
                    }
                    Err(e) => (format!("Failed to list tools of MCP server {}: {}", server_id, e), true),
                }
            }
            .await;
            // 使用完毕, 关闭连接
            ...
            let result = async {
                match service.peer().list_all_tools().await {
                    ...
                    Ok(tools) => match tools.iter().find(|t| t.name.as_ref() == *tool_name) {
                        Some(tool) => {
                            let result = serde_json::json!({"name": tool.name, "description": tool.description, "input_schema": tool.input_schema});
                            (serde_json::to_string(&result).unwrap_or_default(), false)
                        }
        ...
        ["mcp", "server", server_id, "tool", tool_name, "call", json_string] => {
            ...
            let result = async {
                ...
                let arguments = if json_string.is_empty() { Ok(None) } else { serde_json::from_str::<serde_json::Map<String, Value>>(json_string).map(Some).map_err(|e| (format!("Invalid JSON arguments: {}", e), true)) };
                match arguments {
                    Err(r) => r,
                    Ok(arguments) => {
                        ...
                        match service.peer().call_tool(params).await {
                            Ok(tool_result) => {
                                let tool_content_list: Vec<String> = tool_result
                                    ...
                                    .collect();
                                let tool_content = if tool_content_list.len() == 1 { tool_content_list.into_iter().next().unwrap_or_default() } else { serde_json::to_string(&tool_content_list).unwrap_or_default() };
        ...
        _ => (format!("Unknown mcp command: {}", args.join(" ")), true),
    }
}
