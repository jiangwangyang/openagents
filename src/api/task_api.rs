...
pub async fn get_task(State(state): State<AppState>, Path(task_id): Path<i64>) -> Result<Json<serde_json::Value>, AppError> {
    let task = task_repository::get_task(&state.db, task_id).await?.ok_or_else(|| AppError::NotFound("Task not found".to_string()))?;

    // 查询任务的全部阶段对话(含执行 Agent 与消息), 对话与消息均按 id 升序, 每条对话附 running 标记
...
pub async fn add_task(State(state): State<AppState>, Json(req): Json<TaskRequest>) -> Result<Json<i64>, AppError> {
...
pub async fn update_task(State(state): State<AppState>, Path(task_id): Path<i64>, Json(req): Json<TaskRequest>) -> Result<(), AppError> {
    ...
    let updated = task_repository::update_task(&state.db, task_id, &req.title, &req.content, &req.agent_ids, &req.work_dir).await?;
    if !updated {
...
pub async fn start_task(State(state): State<AppState>, Path(task_id): Path<i64>, Json(req): Json<StartTaskRequest>) -> Result<(), AppError> {
    let task = task_repository::get_task(&state.db, task_id).await?.ok_or_else(|| AppError::NotFound("Task not found".to_string()))?;
    agent_repository::get_agent(&state.db, req.agent_id).await?.ok_or_else(|| AppError::NotFound("Agent not found".to_string()))?;
...
pub async fn stop_task(State(state): State<AppState>, Path(task_id): Path<i64>) -> Result<(), AppError> {
...
pub async fn complete_task(State(state): State<AppState>, Path(task_id): Path<i64>, Json(req): Json<CompleteTaskRequest>) -> Result<(), AppError> {
    let task = task_repository::get_task(&state.db, task_id).await?.ok_or_else(|| AppError::NotFound("Task not found".to_string()))?;
    let message = req.message.trim();
    if message.is_empty() {
        return Err(AppError::BadRequest("Message is required".to_string()));
    ...
    if task_service::append_task_user_message(&state, &task, message, false).await?.is_none() {
