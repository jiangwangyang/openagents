...
const I18N = {
    ...
    zh: {
        ...
        mcp: {viewTitle: 'Model Context Protocol 服务', newServer: '+ 新建服务', keyName: '服务唯一名称', keyPlaceholder: '例如：vserver_wind_financial_data', descPlaceholder: '例如：Wind 金融数据服务', protocolArch: '协议类型', urlEndpoint: '连接地址', headersLabel: 'HTTP 请求头（JSON 格式）', command: '执行命令', args: '参数（逗号分隔）', empty: '暂无已注册 MCP 服务', localContext: '本地上下文', testProbe: '测试连接', protocolType: '协议类型', targetUrl: '目标地址', httpHeaders: 'HTTP 请求头（JSON）', cmdExec: '执行命令', exposedCaps: '暴露的能力清单', topologyCrashed: '加载失败', nameRequired: '服务唯一名称为必填项。', headersInvalid: '请求头必须是合法的 JSON 字符串。', regRejected: '创建失败', synced: 'MCP 服务 [{name}] 已保存。', purgeTitle: '删除 MCP 服务', purgeText: '确定要删除 MCP 服务 [{name}] 吗？其提供的工具将不再可用。', connectedNoTools: '连接成功，未暴露任何工具。', noManifest: '未提供说明。', sessionCrashed: '连接被拒绝', nameReadonly: '唯一名称不可修改'},
        config: {viewTitle: '系统运行配置', newProvider: '+ 新建供应商', newProviderCard: '+ 新建模型供应商', providerName: '供应商唯一名称', providerNamePlaceholder: '例如：deepseek', protocolType: '协议类型', baseUrl: '基础地址', secretToken: '密钥（API Key）', settingsFault: '加载失败', endpointMissing: '缺少端点地址', nameRequired: '供应商名称为必填项。', injectionFault: '创建失败', synced: '供应商 [{name}] 已保存。', purgeTitle: '删除模型供应商', purgeText: '确定要删除供应商 [{name}] 吗？使用该供应商的智能体将无法正常工作。', nameReadonly: '唯一名称不可修改'},
