...
pub fn create_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/conversation/list",
            get(conversation_api::list_conversations),
        )
        .route(
            "/conversation/{conversation_id}",
            get(conversation_api::get_conversation),
        )
        .route(
            "/conversation/{conversation_id}",
        ...
        .route(
            "/conversation/{conversation_id}/start",
        ...
        .route(
            "/model-provider/list",
            get(model_provider_api::list_model_providers),
        )
        .route(
            "/model-provider/{provider_id}",
            get(model_provider_api::get_model_provider),
        )
        .route(
            "/model-provider/{provider_id}/model/list",
            get(model_provider_api::list_provider_models),
        )
        .route(
            "/model-provider",
            post(model_provider_api::add_model_provider),
        )
        .route(
            "/model-provider/{provider_id}",
            put(model_provider_api::update_model_provider),
        )
        .route(
            "/model-provider/{provider_id}",
            delete(model_provider_api::delete_model_provider),
        )
        // MCP 服务 CRUD
        .route("/mcp-server/list", get(mcp_server_api::list_mcp_servers))
        .route(
            "/mcp-server/{server_id}",
            get(mcp_server_api::get_mcp_server),
        )
        .route(
            "/mcp-server/streamable-http",
            post(mcp_server_api::add_mcp_streamable_http_server),
        )
        .route(
            "/mcp-server/stdio",
            post(mcp_server_api::add_mcp_stdio_server),
        )
        .route(
            "/mcp-server/{server_id}/streamable-http",
            put(mcp_server_api::update_mcp_streamable_http_server),
        )
        .route(
            "/mcp-server/{server_id}/stdio",
            put(mcp_server_api::update_mcp_stdio_server),
        )
        .route(
            "/mcp-server/{server_id}",
            delete(mcp_server_api::delete_mcp_server),
        )
        .route(
            "/mcp-server/{server_id}/tool/list",
            post(mcp_server_api::list_mcp_server_tools),
        )
        // 技能列表
