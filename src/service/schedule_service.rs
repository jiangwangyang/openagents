...
pub async fn add_schedule(
    ...
    let id =
        schedule_repository::add_schedule(&state.db, name, content, work_dir, cron_expr, agent_id)
            .await?;
...
async fn execute_schedule(state: &AppState, schedule_id: i64) -> anyhow::Result<()> {
    ...
    let started = start_conversation_boxed(
        state,
        conversation_id,
    ...
    if !started {
        tracing::warn!(
            "Schedule {} conversation {} failed to start",
            schedule_id,
            conversation_id
        );
    }
    tracing::info!(
        "Schedule {} triggered conversation {}",
        schedule_id,
        conversation_id
    );
    Ok(())
