...
const I18N = {
    en: {
        ...
        cron: {viewTitle: 'Scheduled Cron Processes', newCron: '+ New Cron', name: 'Task Name', namePlaceholder: 'e.g. Daily Market Scraper', workDirPlaceholder: 'e.g. /home/project/miniclaw', agent: 'Agent', noAgent: '-- None --', minute: 'Min', hour: 'Hour', day: 'Day', month: 'Month', week: 'Weekday', contentLabel: 'Execution Content / Script Context', contentPlaceholder: '# Enter detailed script steps...', empty: 'NO SCHEDULED CRON PROCESSES FOUND', unnamed: 'Unnamed Task', workingDir: 'Working Dir', nextFire: 'Next Fire', triggerSpec: 'Trigger Spec', execContent: 'Execution Content', suspended: 'Suspended', enabled: 'ENABLED', disabled: 'DISABLED', enable: 'Enable', disable: 'Disable', synced: 'CRON TASK [{name}] SAVED.', purgeTitle: 'DELETE CRON TASK', purgeText: 'Delete scheduled task [{name}]? It will no longer run.', purgeFailed: 'DELETE FAILED', formatHint: 'Field order: Min Hour Day Month Weekday. Each accepts digits and * , - /  (e.g. Min=0 Hour=9 = every day at 09:00)', invalidFormat: 'CRON FIELDS MAY ONLY CONTAIN DIGITS AND * , - /', execHistory: 'Execution History', notTriggered: 'NOT TRIGGERED YET. NO EXECUTION RECORDS.'},
        ...
        stream: {purgeTitle: 'DELETE CONVERSATION', purgeText: 'Delete conversation [{name}]? This action cannot be undone.', startFailed: 'START FAILED', configMissing: 'PLEASE SELECT A PROVIDER AND ENTER A MODEL FIRST.', workdirMissing: 'PLEASE SET A WORKING DIRECTORY (WORKDIR) FIRST.', unknownError: 'Unknown Error', thoughtProcess: 'Thought Process', callPrefix: 'Call', tool: 'Tool', toolError: 'Error', toolResult: 'Result', usageIn: 'in', usageOut: 'out', usageCache: 'cache', usageTotal: 'this turn'},
    ...
    zh: {
        ...
        cron: {viewTitle: '定时任务管理', newCron: '+ 新建定时任务', name: '任务名称', namePlaceholder: '例如：每日行情抓取', workDirPlaceholder: '例如：/home/project/miniclaw', agent: '智能体', noAgent: '-- 无 --', minute: '分', hour: '时', day: '日', month: '月', week: '星期', contentLabel: '执行内容 / 脚本上下文', contentPlaceholder: '# 输入详细脚本步骤...', empty: '暂无定时任务', unnamed: '未命名任务', workingDir: '工作目录', nextFire: '下次触发', triggerSpec: '触发规则', execContent: '执行内容', suspended: '已暂停', enabled: '已启用', disabled: '已禁用', enable: '启用', disable: '禁用', synced: '定时任务 [{name}] 已保存。', purgeTitle: '删除定时任务', purgeText: '确定要删除定时任务 [{name}] 吗？删除后将不再执行。', purgeFailed: '删除失败', formatHint: '字段顺序：分 时 日 月 星期。每个字段支持数字及 * , - / 符号（如 分=0 时=9 表示每天 09:00）', invalidFormat: '定时字段只能包含数字和 * , - / 符号。', execHistory: '执行记录', notTriggered: '尚未触发执行，暂无记录。'},
        ...
        input: {cwdTitle: '设置目录上下文', unset: '未设置', agentTitle: '选择智能体', providerTitle: '选择供应商', modelTitle: '选择模型', thinkingTitle: '思考模式', placeholder: '输入指令...', execute: '发送', charLimit: '已达输入上限（{max} 字符），超出内容已被丢弃。', contextLocked: '会话已创建，工作目录与智能体不可修改。', cwdLabel: '工作目录：', modelListUnavailable: '无法获取模型列表，可直接输入模型名称。'},
        modal: {workspaceContext: '工作目录上下文', pathPlaceholder: '输入路径...', go: '跳转', recentWorkdirs: '最近工作目录', confirmPath: '确认路径', removeHistory: '从历史中移除'},
        stream: {purgeTitle: '删除会话', purgeText: '确定要删除会话 [{name}] 吗？该操作不可恢复。', startFailed: '启动失败', configMissing: '请先选择供应商并填写模型。', workdirMissing: '请先设置工作目录（WORKDIR）。', unknownError: '未知错误', thoughtProcess: '思考过程', callPrefix: '调用', tool: '工具', toolError: '错误', toolResult: '结果', usageIn: '输入', usageOut: '输出', usageCache: '缓存', usageTotal: '本轮'},
