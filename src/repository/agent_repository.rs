...
// 新增 Agent, 返回自增 id
pub async fn add_agent(pool: &SqlitePool, name: &str, description: &str, prompt: &str, model_provider_id: i64, model: &str, thinking: &str) -> Result<i64, sqlx::Error> {
...
pub async fn update_agent(pool: &SqlitePool, agent_id: i64, name: &str, description: &str, prompt: &str, model_provider_id: i64, model: &str, thinking: &str) -> Result<bool, sqlx::Error> {
    let now = chrono::Local::now().to_rfc3339();
