...
pub async fn list_mcp_servers(pool: &SqlitePool) -> Result<Vec<McpServerEntity>, sqlx::Error> {
    sqlx::query_as::<_, McpServerEntity>("SELECT id, name, description, protocol_type, url, headers, command, args, create_time, update_time FROM t_mcp_server ORDER BY id").fetch_all(pool).await
}

// 按 id 查询 MCP 服务
pub async fn get_mcp_server(pool: &SqlitePool, server_id: i64) -> Result<Option<McpServerEntity>, sqlx::Error> {
    sqlx::query_as::<_, McpServerEntity>("SELECT id, name, description, protocol_type, url, headers, command, args, create_time, update_time FROM t_mcp_server WHERE id = ?").bind(server_id).fetch_optional(pool).await
}
...
pub async fn delete_mcp_server(pool: &SqlitePool, server_id: i64) -> Result<bool, sqlx::Error> {
    let result = sqlx::query("DELETE FROM t_mcp_server WHERE id = ?").bind(server_id).execute(pool).await?;
    Ok(result.rows_affected() > 0)
}
